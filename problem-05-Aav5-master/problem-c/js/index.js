...
function renderTaskList() {
  ...
  state.taskList.forEach(function(task) {
    let item = createTaskItemElement(task);
...
function addNewTask() {
  let idNumber = state.taskList[state.taskList.length-1].id + 1;
  let desc = state.inputtedText;
  state.taskList.push({id:idNumber, description:desc, complete:false});
  state.inputtedText = "";
  renderTaskList();
}
